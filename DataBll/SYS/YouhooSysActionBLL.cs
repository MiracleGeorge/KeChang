...
{
    /// <summary>
    /// 业务逻辑类：YouhooSysActionBLL
    /// 时间：2015/4/8 15:49:38
    /// </summary>
    public class YouhooSysActionBLL
    {
        private readonly YouhooSysActionDAL dal = new YouhooSysActionDAL();
        public static string CacheName = "YouhooSysActionDAL-";

        public YouhooSysActionBLL()
        { }
        #region  成员方法
        /// <summary>
        /// 是否存在该记录
        ...
        {
            string strCache = string.Format(CacheName + "GetModel-{0}", actionId);
            YouhooSysActionModel model = CacheHelper.GetCache(strCache) as YouhooSysActionModel;
            if (model == null)
            {
                model = dal.GetModel(actionId);
