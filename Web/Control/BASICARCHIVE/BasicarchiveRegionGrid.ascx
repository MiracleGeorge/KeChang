<%@ Control Language="C#" AutoEventWireup="true" CodeFile="BasicarchiveRegionGrid.ascx.cs" Inherits="YouHoo.Web.Control.BASICARCHIVE.BasicarchiveRegionGrid" %>
<%@ Import Namespace="YouHoo.DataTools" %>

<input type="hidden" id="bllName" value="YouhooBasicarchiveRegionBLL" />
		...
		<ItemTemplate>
			<tr>
				...
				<td align="center" class="operate">
					<a href="javascript:dialogDetail('/BASICARCHIVE/BasicarchiveRegionDetail.aspx?PageIndex=<%#PageIndex %>&ReturnUrl=<%=DataRequest.UrlEncode(Request.RawUrl) %>&id=<%# Eval("id") %>')">查看</a>
					<asp:LinkButton ID="lnk_update" Text="修改" Visible='<%#UpdatePower %>' OnClientClick='<%# "return dialogUpdateSingle(&#39;/BASICARCHIVE/BasicarchiveRegionEdit.aspx?PageIndex="+PageIndex+"&ReturnUrl="+DataRequest.UrlEncode(Request.RawUrl)+"&id="+Eval("id")+"&#39;)" %>'
