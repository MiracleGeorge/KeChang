<%@ Control Language="C#" AutoEventWireup="true" CodeFile="BasicarchiveSupportwayGrid.ascx.cs" Inherits="YouHoo.Web.Control.BASICARCHIVE.BasicarchiveSupportwayGrid" %>
<%@ Import Namespace="YouHoo.DataTools" %>

<input type="hidden" id="bllName" value="YouhooBasicarchiveSupportwayBLL" />
		...
		<ItemTemplate>
			<tr>
				...
				<td align="center" class="operate">
					<a href="javascript:dialogDetail('/BASICARCHIVE/BasicarchiveSupportwayDetail.aspx?PageIndex=<%#PageIndex %>&ReturnUrl=<%=DataRequest.UrlEncode(Request.RawUrl) %>&id=<%# Eval("id") %>')">查看</a>
					<asp:LinkButton ID="lnk_update" Text="修改" Visible='<%#UpdatePower %>' OnClientClick='<%# "return dialogUpdateSingle(&#39;/BASICARCHIVE/BasicarchiveSupportwayEdit.aspx?PageIndex="+PageIndex+"&ReturnUrl="+DataRequest.UrlEncode(Request.RawUrl)+"&id="+Eval("id")+"&#39;)" %>'
