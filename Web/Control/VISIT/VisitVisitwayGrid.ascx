		...
		<ItemTemplate>
			<tr>
				...
				<td align="center" class="operate">
					<a href="javascript:dialogDetail('/VISIT/VisitVisitwayDetail.aspx?PageIndex=<%#PageIndex %>&ReturnUrl=<%=DataRequest.UrlEncode(Request.RawUrl) %>&visit_way_id=<%# Eval("visit_way_id") %>')">查看</a>
					<asp:LinkButton ID="lnk_update" Text="修改" Visible='<%#UpdatePower %>' OnClientClick='<%# "return dialogUpdateSingle(&#39;/VISIT/VisitVisitwayEdit.aspx?PageIndex="+PageIndex+"&ReturnUrl="+DataRequest.UrlEncode(Request.RawUrl)+"&visit_way_id="+Eval("visit_way_id")+"&#39;)" %>'
